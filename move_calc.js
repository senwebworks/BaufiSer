...
files.forEach(f => {
    try {
        ...
        // 1. Remove the live-calc-result from Step 9
        const step9start = html.indexOf('<div class="form-step" id="step-9">');
        const step10start = html.indexOf('<div class="form-step" id="step-10">');
        
        if (step9start !== -1 && step10start !== -1) {
            let step9Html = html.substring(step9start, step10start);
            
            const calcStart = step9Html.indexOf('<div class="live-calc-result"');
            const actionsStart = step9Html.indexOf('<div class="form-actions">');
            
            if (calcStart !== -1 && actionsStart !== -1 && calcStart < actionsStart) {
                step9Html = step9Html.substring(0, calcStart) + step9Html.substring(actionsStart);
                html = html.substring(0, step9start) + step9Html + html.substring(step10start);
            }
        }
        
        // 2. Replace Step 12 content
        ...
        if (step12start !== -1 && stepSuccessStart !== -1) {
            const newStep12Html = `                        <!-- Step 12: Prüfung starten -->
            ...
            html = html.substring(0, step12start) + newStep12Html + html.substring(stepSuccessStart + 48); // skip original <!-- Erfolgsmeldung --> if it was in the snippet
            // Wait, stepSuccessStart is at '<div class="form-step" id="step-success">'
            // The comment <!-- Erfolgsmeldung --> is before it. 
            // So html.substring(stepSuccessStart) actually covers from <div id="step-success" onwards.
            // Oh right, what about the comment? We just inject the comment in newStep12Html above.
            // But wait, stepSuccessStart = html.indexOf('<div class="form-step" id="step-success">');
            // If I append html.substring(stepSuccessStart), it's completely fine.
            html = html.substring(0, step12start) + newStep12Html.replace('                        <!-- Erfolgsmeldung -->\n', '') + html.substring(stepSuccessStart);
        }
        
        fs.writeFileSync(f, html);
        console.log('Moved calc in ' + f);
    } catch (e) {
